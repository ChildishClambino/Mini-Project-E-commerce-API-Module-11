...
class Order(db.Model):
    __tablename__ = 'order'  # Corrected table name
    id = db.Column(db.Integer, primary_key=True)
...
class OrderProduct(db.Model):
    order_id = db.Column(db.Integer, db.ForeignKey('order.id'), primary_key=True)  # Corrected table name
...
class OrderSchema(Schema):
    customer_id = fields.Int(required=True)
    order_date = fields.Date(required=True)  # Add this line to include order_date
...
def place_order():
    ...
    try:
        ...
        print('Validation error:', err.messages)  # Add this line to log validation errors
        return jsonify(err.messages), 400
    ...
    new_order = Order(customer_id=customer_id, order_date=validated_data['order_date'])
